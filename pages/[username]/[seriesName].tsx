...
const TimeseriesPage = ({ thisSeries, canEdit, isOwner }: { thisSeries: DatedObj<SeriesObj>, canEdit: boolean, isOwner: boolean }) => {
    ...
    function onSubmit() {
        setIsLoading(true);
        axios.post("/api/note", { seriesId: thisSeries._id, date: date, body: body, })
            .then((res) => {
    ...
    function onDelete(noteId: string) {
        setIsLoading(true)
        axios.delete(`/api/note`, { data: { id: noteId, } })
            .then(res => {
                console.log(res.data.message);
                setIter(iter + 1);
            })
                ...
                <>
                    ...
                    {(notesData && notesData.data) ? notesData.data.length > 0 ? notesData.data.map(note =>
                        <div className="mb-16" key={note._id}>
                            ...
                            {isOwner && <ContextMenu id={note._id} className="bg-white rounded-md shadow-lg z-10 cursor-pointer">
